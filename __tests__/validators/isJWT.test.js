const {validators: {isJWT}} = require('../../dist/build.common');

describe('Valid JWT should be true', () => {
  test.each([
  ...
  ])('isJWT("%s") = %s', (a, expected) => {
    expect(isJWT(a)).toBe(expected);
  });
});

describe('Invalid JWT should be false', () => {
  test.each([
