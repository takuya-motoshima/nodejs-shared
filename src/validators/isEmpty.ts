...
export default (value: any, options: IsEmptyOptions = {}): boolean => {
  ...
  const trimmedValue = mergedOptions.ignoreWhitespace ? stringValue.trim() : stringValue;
  return validator.isEmpty(trimmedValue, {ignore_whitespace: mergedOptions.ignoreWhitespace});
}